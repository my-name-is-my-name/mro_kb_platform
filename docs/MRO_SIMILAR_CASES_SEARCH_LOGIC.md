...
- `case_id`;
- `Score`: относительный технический score ранжирования, а не вероятность и не цена;
- заказчика, тип ВС как display-only поле и статус в компактной ячейке заявки;
- описание;
- расширенную колонку `почему похожа` без дублирующей второй таблицы источников;
- что отличается или что надо проверить;
- короткий статус документов: `есть N`, `нет`, `проверить связь`;
- пригодность аналога для будущей оценки стоимости;
...
- HTTP endpoint: `/api/com-offers/registry/{case_id}` отдает HTML-страницу для браузера;
- raw Markdown: `/api/com-offers/registry/{case_id}?format=md`;
...
Колонка `Оценка стоимости` не означает рассчитанную цену. Это индикатор, можно ли использовать историческую заявку как comparable для следующего этапа оценки стоимости:

- `годится (score/6)`: есть достаточно признаков, например принятый статус, дата КП, тип работ и trusted evidence;
- `только ориентир (score/6)`: аналог полезен технически, но данных недостаточно для уверенной стоимостной базы.

Колонка `Score` нужна для диагностики ранжирования: первое число - итоговый retrieval/rerank score текущего алгоритма, `R` - score внешнего reranker, если он был применен. Эти числа можно сравнивать внутри одной выдачи, но нельзя трактовать как вероятность совпадения.
